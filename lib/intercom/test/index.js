...
describe('Intercom', function(){
  ...
  describe('.group()', function(){
    it('should be able to group correctly', function(done){
      ...
      var userRequest = {};
      secondGroup.input.traits.created_at = time(new Date(secondGroup.input.traits.created_at));

      payload.user_id = secondGroup.input.userId;
      payload.last_request_at = time(secondGroup.input.timestamp);

      payload.companies = [{
        company_id: secondGroup.input.groupId,
        custom_attributes: secondGroup.input.traits,
        name: secondGroup.input.traits.name,
        remote_created_at: secondGroup.input.traits.created_at
      }];

      payload.companies[0].custom_attributes.id = secondGroup.input.groupId;

      payload.custom_attributes = {
        companies: [payload.companies[0].custom_attributes],
        id: secondGroup.input.userId
      };
