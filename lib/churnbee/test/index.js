...
describe('ChurnBee', function(){
  ...
  describe('.track()', function(){
    ...
    it('should error on invalid key', function(done){
      var date = new Date;
      var iso = encodeURIComponent(date.toISOString());

      test
        .set({ apiKey: 'key' })
        .track({ event: 'register', userId: 'user-id', timestamp: date })
        .expects(401)
        .error('cannot GET /v1/user/user-id/?accessToken=key&dateTime=' + iso + '& (401)', done);
    });
  });
});
